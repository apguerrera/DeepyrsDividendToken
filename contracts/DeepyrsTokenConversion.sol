...
contract ERC20 is IERC20, Owned {
  ...
  address public convertedTokenAddress;
...
// ------------------------------------------------------------------------
  ...
  function userDeposit (_amount) public onlyOwner {
    ...
    emit DepositReceived(now, msg.sender, amount);
  }

  function lockDeposit (_amount) public onlyOwner {
    require(IERC20(convertedTokenAddress).transferFrom(msg.sender, address(this), _amount));

  }

  function cancelDeposit (_amount) public {
    _burn(msg.sender, _amount);

  }

  function convertDeposit (_account, _amount) public {
    // Check canConvert
    require(IERC20(convertedTokenAddress).transfer(_account, _amount));

  }

  function _canConvert (_account, _amount) internal returns (bool) {
  ...
  function transferAnyERC20Token(address tokenAddress, uint tokens) public onlyOwner returns (bool success) {
      if (tokenAddress == convertedTokenAddress) { return false; }
